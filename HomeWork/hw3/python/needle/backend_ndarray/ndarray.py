...
class NDArray:
    ...
    def __init__(self, other, device=None):
        ...
        # 如果other是NDArray类型，那么就是复制
        if isinstance(other, NDArray):
            # create a copy of existing NDArray
            if device is None:
                device = other.device
            self._init(other.to(device) + 0.0)  # this creates a copy
        # 如果other是numpy.ndarray类型，那么就调用make，construct一个新的array
    ...
    def fill(self, value):
        ...
        ## 这里调用的fill好像是device特异的fill，在src的ndarray_backend_cpu.py中
    ...
    def permute(self, new_axes):
        ...
        # ### BEGIN YOUR SOLUTION
        # new_shape = list(self.shape[i] for i in new_axes)
        # new_strinds = list(self.strides[i] for i in new_axes)
        # 这里为甚恶魔是tuple，有什么深意吗
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # 这里假设new_shape的维度和self的维度一样
        assert all(new_shape[i] == self.shape[i] or self.shape[i] == 1 for i in range(len(self.shape)))
        
        old_strides = NDArray.compact_strides(new_shape)
        new_strides = tuple([old_strides[i] if self.shape[i] != 1 else 0 for i in range(len(self.shape))])
        return self.as_strided(new_shape, new_strides)
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        print(idxs)
        new_shape = tuple([(i.stop-i.start)//i.step+1 for i in idxs])
        new_strides = tuple([self.strides[i]*j.step for i,j in zip(range(len(self.shape)),idxs)])
        offset = np.sum([i.start*j for i,j in zip(idxs,self.strides)])
        return self.make(new_shape,new_strides,self.device,self._handle,offset=offset)      
        ### END YOUR SOLUTION
